...
#
# Install VOXD, an offline voice-to-text dictation tool.
#
...
if [[ -z "${latest_version}" ]]; then
  print_error "Failed to retrieve the latest version of VOXD. Skipping."
elif [[ "${installed_version}" == "${latest_version}" ]]; then
  print_info "Latest version of VOXD, v${latest_version}, is already installed. Skipping."
else

  print_info "Installing/updating VOXD from .deb package."
  ...
  # Install VOXD from the downloaded Debian package. Fix (-f) any broken
  # dependencies - this is RECOMMENDED for packages installed directly from
  # Debian packages.
  superdo apt-get install -f -y ./voxd_*_amd64.deb
  ...
  print_success "VOXD installed successfully."
