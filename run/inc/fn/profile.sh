...
#
# Output:
#   The rank: 0 for `cli`, 1 for `tui`, 2 for `gui`, -1 for anything else.
#
profile_rank() {
  case "$1" in
    cli) echo 0 ;;
...
# is_cli_profile - Test whether the run is scoped to the `cli` profile.
#
...
# cli profile is simply not called with `cli_step`.
#
# Returns:
#   0 if `--profile=cli` was passed, 1 otherwise.
#
is_cli_profile() {
  [[ "${profile:-tui}" == "cli" ]]
}
