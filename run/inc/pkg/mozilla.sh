...
#
# Add Mozilla's package repository, and correspondining GPG key, to APT.
#
# https://support.mozilla.org/en-US/kb/install-firefox-linux
#

print_step "Adding Mozilla's official package repository."

if [[ -f /etc/apt/keyrings/packages.mozilla.org.asc ]] \
  && [[ -f /etc/apt/sources.list.d/mozilla.list ]] \
  && [[ -f /etc/apt/preferences.d/mozilla ]]; then
  print_info "Mozilla's package repository is already configured. Skipping."
  return 0
fi
...
# Add the Mozilla APT repository to your sources list - only if it is not already present.
...
# Configure APT to prioritize the Mozilla repository over other package sources.
...
print_success "Mozilla's package repository added to APT sources."
