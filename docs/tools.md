...
cumulative — `agent` ⊆ `tui` ⊆ `gui` — and a ✅ means the program is installed
in that profile:
...
- **TUI** — `./run/install`, the default. Everything in Agent, plus the tools
...
[`run/inc/fn/install-steps.sh`](../run/inc/fn/install-steps.sh) — `agent_step`,
