...
- [x] Print the installed version at the end of every install step where the tool exposes `--version`. The `install-step` skill already mandates this rule; audit each existing script in [run/inc/](run/inc/) for compliance and add the missing `echo`s.
